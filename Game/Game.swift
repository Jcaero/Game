...
class Game {
    ...
    private func fight() {
        ...
        print("\(self.player[1].name) gagne la partie ")
        showPlayersTeam()
    }
    ...
    private func selectCharacterAlive(player: Player) -> Int {
        ...
        number = selectedNumber(in: player.nombreOfCharacterAlive())
        print("vous avez selectionné \(player.team[number-1].name), un \(player.team[number-1].type)")
        return number-1
    }
